...
class MultiSpeakerTTSDataset(Dataset):
    ...
    TTSDataset과 동일한 텐서 구조를 사용하되, 각 샘플에 speaker_id 필드가
    필요하다. ref_audio 처리는 collate_fn이 아닌 precompute_speaker_embeddings
    함수에서 학습 시작 전에 수행되므로 이 클래스에서는 ref_mel을 반환하지 않는다.

    입력 JSONL 필수 필드:
        audio, text, ref_audio, speaker_id, audio_codes

    권장 사항:
        같은 speaker_id를 가진 모든 샘플에서 동일한 ref_audio 파일을 사용할 것.
    """
    ...
    def __getitem__(self, idx):
        ...
        speaker_id  = item["speaker_id"].lower()
